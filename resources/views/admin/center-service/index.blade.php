...
@section('content')
    <x-admin.page-header title="Services" :breadcrumb="[['label' => 'Dashboard', 'link' => route('admin.dashboard')], ['label' => 'Services']]" />
        ...
        const tableColumns = [{
            ...
            },
            {
                ...
                render: (data, type, row) => {
                    let url1 = "{{ route('admin.document.index', ':id') }}".replace(':id', row.slug);
                    let url2 = "{{ route('admin.document-group.index', ':id') }}".replace(':id', row.slug);
